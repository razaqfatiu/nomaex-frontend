...
export const getOneProduct = (productId) => {
  return async (dispatch, getState) => {
    try {
      let loading = getState().product.loading = true
      const getOneProduct = await axios.get(`${url}/api/v1/products/${productId}`, { withCredentials: true })
      loading = getState().product.loading = false
...
export const getProductsByCategory = (categoryId) => {
  return async (dispatch, getState) => {
    try {
      const getProductsByCategory = await axios.get(`${url}/api/v1/category/${categoryId}`, { withCredentials: true })
